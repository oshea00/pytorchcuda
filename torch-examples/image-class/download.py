...
def download_image(url, klass, data_type, index, csv_count):
    print(f"Downloading {index} of {csv_count} for {url} {klass} {data_type}")
...
if __name__ == "__main__":
    ...
    csv_count = len(imagesDF)
    urls = zip(imagesDF["url"], imagesDF["class"], imagesDF["type"])
    for index, (url, klass, data_type) in enumerate(urls):
        download_image(url, klass, data_type, index, csv_count)
